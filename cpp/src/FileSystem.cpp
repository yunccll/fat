...
Status FileSystem::flushFat(){
    ...
    for(size_t i = 0; i < len; ++i){
        auto s = device->write((void*)(fsInfo->firstSectorOfFatBackup() + i), Slice(fatBuffer.c_str() + i * fsInfo->bytesPerSector(), fsInfo->bytesPerSector()));
        if(!s) return s;
    }
    return Status::OK();
}
